...
**The site runs one radius, `0.75rem`, and all three daisyUI radius tokens are held equal to it.** daisyUI splits them by component family — `--radius-selector` for badges and toggles, `--radius-field` for buttons and inputs, `--radius-box` for cards, alerts and modals — and they were `1rem`/`2rem`/`0.25rem` until 2026-08, which gave a chip, a button and a card three different corners. Holding them equal is the whole mechanism; changing one alone re-introduces the split. Five things sit outside the tokens and have to be kept in step by hand:

- **Hand-written panels take the radius from the markup, via `rounded-box`.** `.post-card` and `.library-card` are not daisyUI components, so nothing rounds them for free — each carries the utility in its template (`post-card.html`, `library/single.html`). Declaring `border-radius` in the `@layer components` block instead would be dead code: utilities sit in a later layer and win regardless. The trade is that a new panel which forgets the class renders square rather than inheriting anything.
- **An inset child steps the radius down, via `--radius-inset` — the single exception to the one-radius rule, and it exists to *preserve* the one radius.** Two nested boxes carrying the same `border-radius` are not concentric: the panel's corner curves from its own outer edge, so 9px in — `padding: 0.5rem` plus the 1px border — only a 3px corner is left, and a 12px cover corner inside that widens the frame from 9px along the straight edges to 12.7px across the diagonal. That gap is what reads as "the corners don't match". Each panel declares `--radius-inset: max(0px, calc(var(--radius-box) - 0.5rem - 1px))` beside the padding it is derived from, and `.post-card__img`, `.library-card__img` and `.library-card__placeholder` take their corners from it. These three are therefore the one place a cover must **not** carry `rounded-box` — the utility would win over the rule and put the mismatch back. Change a panel's padding or border and its `--radius-inset` has to move with it.
...
- **Circles are exempt.** `.toot-profile__avatar` stays `9999px`; an avatar reads as a person. `.polaroid` is exempt in the other direction — a print has square corners.
- **`rounded-box` is the only `rounded-*` to reach for.** It is the one that maps to the token; `rounded-sm`/`rounded-t`/`rounded-lg` and friends carry Tailwind's own scale and will silently reintroduce a second radius. It is also already in `hugo_stats.json`, so it costs no two-build lag. Where no single utility fits — there is no `rounded-t-box` — let a parent's radius clip the child with `overflow-hidden`, which is what `now-top-artists.html` does.
...
- **The tokens are built to the same rule as the theme blocks**, and live next to `--color-heading` in `main.css` rather than inside `@plugin "daisyui/theme"`, which only understands its own key names. Sunrise is the *dark* member with the paper `-content`; midnight is the *light* member with `-content` mixed 10% into `base-100`. Measured against `base-200`: ≥4.62 sunrise / ≥5.46 midnight as plain text, ≥5.53 through `.badge-tint`, ≥4.99 solid.
...
- **Each hue needs a `badge-*` class, and it is exactly two declarations.** DaisyUI's own `badge-primary` compiles to `--badge-color` + `--badge-fg`, so matching that pair gives both chip shapes for free: tinted when combined with `.badge-tint`, solid when not. They sit **unlayered** with `.badge-tint` for the same cascade reason, and being hand-written CSS they also sidestep the `hugo_stats.json` two-build lag that forces the `@source inline(...)` safelist.
...
Blog category chips stay **solid** while tags stay tinted. That contrast is now the *only* thing keeping categories reading above tags, since both families carry colour — don't tint the categories.
...
The cap was `max-w-prose` until 2026-08 — 65ch, which in Raleway at the site's 16px root works out to ~637px. It was widened to mimic willianjusten.com.br, whose text column measures 915px; `max-w-4xl` is the nearest step on Tailwind's container scale (`max-w-5xl` is 1024px, `max-w-3xl` 768px). Two things follow. **The root font-size is 16px, the browser default** — a comment above `.site-shell` claims 18px is set on `html`, and nothing in the repo sets it; don't size anything off that claim. And **896px at a 16px body is ~92 characters a line**, past the usual 65–75 band: the reference site buys its width with a 20px body font, which this site does not have. The step down if it reads too long is `max-w-3xl` (~78ch), not a return to `max-w-prose`.
...
- **`$post` must be captured before `{{ with .Params.cover }}`.** Covers are page-bundle resources (bare filenames), so `responsive-image.html` needs `"page" $post` to resolve them — but inside the `with`, `.` is the cover *string*. This is the same dance the old featured card did at `blog/list.html:71`.
- **The category chip is a `<span>`, never an `<a>`.** The whole card is a single anchor, so a nested link would be invalid HTML and would split the card's one tab stop. It is also **solid**, not `badge-tint` — that contrast is the only thing keeping categories reading above tags. Only the *first* category renders, and it is read with a guarded `{{ with $post.Params.categories }}{{ $cat = index . 0 }}{{ end }}`: `index` on a nil slice is a build failure.
...
- **The status goes in the meta row, not on the title.** The row is `date · reading time · status`, and the status is built from `post-status-icon.html` plus the `status_*` string rather than by calling `post-status-badge.html` — that renders a *coloured chip*, which is loud beside two muted meta items when the cover already carries a solid category chip. The emoji is `aria-hidden` with its label next to it, so a screen reader says "Seed" once instead of also announcing the glyph. `post-status-emoji.html` (the old inline title prefix) is untouched and still used by `page-list.html` and `prev-next-nav.html`. In pt-br this row is the tightest fit in the design — `10 de abr. de 2026 · 8 min de leitura · Em cultivo` — and wraps to two lines on a grid card; `.post-card__meta` sets `flex-wrap: wrap` for exactly that.
- **`icons/meta/calendar.html` and `icons/meta/clock.html` carry no `w-4 h-4`**, unlike the root-level icons. Tailwind sizing utilities compile into `@layer utilities` and beat the `@layer components` rule that styles the meta row — the same cascade trap `.polaroid` documents. Size lives in `.post-card__meta svg`. This is a third icon convention alongside the root files and `icons/alert/`; keep the pair in step.
- **`.post-card__sep` is mixed from `base-content`, not `base-300`.** The card's hover state *is* `base-300`, so a `base-300` divider would vanish exactly when the card is pointed at.
- **The panel is `base-200` on a `base-300` hairline**, copied from `.library-card` — cover art is an object and a surface under it says so. The reference design was a white card on a grey page, which does not translate: in sunrise `base-100` *is* the reading column, so a `base-100` card disappears into it.
- **The panel takes `rounded-box` from the markup; the cover takes `--radius-inset` from the CSS.** Giving the cover `rounded-box` too is the obvious move and is wrong — see the concentricity note under **Theming**. The panel and the cover are the same shape at different offsets, not the same radius.

The two line-clamps are hand-written `-webkit-box` CSS rather than `line-clamp-2`, for the usual reason — Hugo writes `hugo_stats.json` *during* the build, so a newly-used utility takes two builds to be emitted. The grid classes are safe as utilities only because `layouts/snippets/list.html` already puts them in the stats file.

`layouts/partials/page-list.html` survives as `_default/list.html`'s renderer and is deliberately *not* card-shaped: it serves sections with no cover art. `dated-list-item.html` was the /blog archive row and was deleted when the grid replaced it.
...
**`updated` is optional and only renders when it is a *later day* than `date`.** `layouts/blog/single.html` puts it inline in the meta row between the published date and the status badge, labelled with `post_updated_label` — a shorter key than the `last_updated_label` used by the accent-bar block in `partials/header.html` and `blog/list.html`, which those two keep. The comparison is on `2006-01-02` strings, not `.After`: a date-only `updated:` parses to UTC midnight, so against a same-day `date:` carrying a `-03:00` offset an instant comparison reads it as *earlier* and silently drops it. The consequence is that a same-day revision cannot be shown — which is the right trade, since the row displays day granularity anyway. Guard every read with `{{ with }}`: `archetypes/blog.md` emits `updated: ""` and `time.AsTime ""` fails the build. Nothing in the list views reads it — the homepage row, the featured card, the archive and `blog-posts.html`'s ordering all key off `.Date`, so an updated post does not resurface.
