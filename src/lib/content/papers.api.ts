...
export function getPublicationEntries(): CustomBibTexEntry[] {
  ...
  const customEntries = bibTexEntries.entries_raw.map<CustomBibTexEntry>(
    (entry: BibEntry) => ({
      id: entry._id,
      ...
      author: (entry.getFieldAsString("author") as string)
        .split("and")
...
export function getPublicationEntriesSorted(): CustomBibTexEntry[] {
  return getPublicationEntries().sort((firstEntry, secondEntry) =>
    firstEntry.year > secondEntry.year ? -1 : 1
  );
}
