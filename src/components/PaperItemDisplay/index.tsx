...
const PaperItemDisplay: React.FC<PaperItemDisplayProps> = ({ item }: PaperItemDisplayProps) => {
  return (
    <details className="shadow-md bg-slate-200 dark:bg-slate-800 border dark:border-slate-700" key={item.id}>
      <summary className="font-semibold line-clamp-1 p-4">{item.title}</summary>
      <div className="text-slate-600 dark:text-slate-400 px-4 mb-2">{item.media}</div>
      <p className="bg-slate-50 dark:bg-slate-700 text-sm leading-relaxed p-4">{item.abstract}</p>
